...
int App::main (int argc, char **argv) {
  ...
  const char *vectors_file_path = 0; // optional custom vectors file for --ortho
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp (argv[i], "-h") || !strcmp (argv[i], "--help") ||
      ...
      use_complex = true;
      std::cout << "c Using complex vector operations" << std::endl;
    } else if (!strcmp (argv[i], "--vectors-file")) {
      if (++i == argc)
        APPERR ("argument to '--vectors-file' missing");
      else
        vectors_file_path = argv[i];
      ...
      if (!force_writing && most_likely_existing_cnf_file (argv[i]))
        APPERR ("proof file '%s' most likely existing CNF (use '-f')", argv[i]);
      else if (!File::writable (argv[i]))
        APPERR ("proof file '%s' not writable", argv[i]);
      else
        proof_path = argv[i];
    } else
  ...
  // Enable proof tracing if a proof file was provided (before parsing/solving).
  if (proof_path) {
    solver->section ("opening proof trace");
    solver->message ("writing proof to %s'%s'%s", tout.green_code (), proof_path, tout.normal_code ());
    bool opened = solver->trace_proof (proof_path);
    if (!opened) APPERR ("could not open proof file '%s'", proof_path);
  }
  ...
  if (incremental) {
    ...
    std::unique_ptr<SymmetryBreaker> sb(new SymmetryBreaker(solver, order, unembeddable_check, partition_size, std::string(vectors_file_path ? vectors_file_path : "")));
    sb->set_use_master_graph(use_master);
    sb->set_print_statement(print_statement);
    sb->set_use_orthogonality(use_ortho);
    sb->set_use_complex(use_complex);
    sb->set_no_print(no_print);
    
    // Set binary format based on solver option
    bool use_binary = solver->get("binary");
    sb->set_binary_format(use_binary);
    
    // Automatically set permutation filename based on proof path
    if (proof_path) {
      std::string proof_str(proof_path);
      std::string perm_filename = proof_str;
      // Replace .drat extension with .perm
      size_t dot_pos = perm_filename.find_last_of('.');
      if (dot_pos != std::string::npos && perm_filename.substr(dot_pos) == ".drat") {
        perm_filename = perm_filename.substr(0, dot_pos) + ".perm";
      } else {
        perm_filename += ".perm";
      }
      sb->set_permutation_filename(perm_filename);

      // Set orthogonality filename
      std::string ortho_filename = proof_str;
      dot_pos = ortho_filename.find_last_of('.');
      if (dot_pos != std::string::npos && ortho_filename.substr(dot_pos) == ".drat") {
        ortho_filename = ortho_filename.substr(0, dot_pos) + ".ortho";
      } else {
        ortho_filename += ".ortho";
      }
      sb->set_orthogonality_filename(ortho_filename);
    }

    sb->print_orthogonality_status();
  ...
  // Close proof trace before printing result to ensure proof is flushed.
  if (proof_path) {
