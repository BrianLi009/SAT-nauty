...
class DratTracer : public FileTracer {
  ...
  void drat_add_comment (const vector<int> &);
  void drat_add_orthogonality_clause (const vector<int> &);
...
public:
  ...
  void add_comment (const vector<int> &clause);

  // Add orthogonality clause to DRAT proof
  void add_orthogonality_clause (const vector<int> &clause);
