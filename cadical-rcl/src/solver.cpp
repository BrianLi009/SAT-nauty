...
void Solver::add_proof_comment (const std::vector<int> &clause) {
  TRACE ("add_proof_comment");
  REQUIRE_VALID_OR_SOLVING_STATE ();
  DratTracer* drat_tracer = internal->get_drat_tracer();
  if (drat_tracer) {
    drat_tracer->add_comment(clause);
  }
  LOG_API_CALL_END ("add_proof_comment");
}

void Solver::add_proof_orthogonality_clause (const std::vector<int> &clause) {
  TRACE ("add_proof_orthogonality_clause");
  REQUIRE_VALID_OR_SOLVING_STATE ();
  DratTracer* drat_tracer = internal->get_drat_tracer();
  if (drat_tracer) {
    drat_tracer->add_orthogonality_clause(clause);
  }
  LOG_API_CALL_END ("add_proof_orthogonality_clause");
}

void Solver::add_trusted_clause (const std::vector<int> &clause) {
  TRACE ("add_trusted_clause");
  REQUIRE_VALID_OR_SOLVING_STATE ();
  
  // Trace to DRAT proof as trusted clause (like cadical-ks)
  DratTracer* drat_tracer = internal->get_drat_tracer();
  if (drat_tracer) {
    drat_tracer->add_comment(clause); // Use 't' prefix like cadical-ks
  }
  
  LOG_API_CALL_END ("add_trusted_clause");
}

void Solver::add_trusted_orthogonality_clause (const std::vector<int> &clause) {
  TRACE ("add_trusted_orthogonality_clause");
  REQUIRE_VALID_OR_SOLVING_STATE ();
  
  // Trace to DRAT proof as orthogonality clause (like cadical-ks)
  DratTracer* drat_tracer = internal->get_drat_tracer();
  if (drat_tracer) {
    drat_tracer->add_orthogonality_clause(clause); // Use 'o' prefix
  }
  
  LOG_API_CALL_END ("add_trusted_orthogonality_clause");
}
