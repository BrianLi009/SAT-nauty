...
void DratTracer::drat_add_comment (const vector<int> &clause) {
  if (binary)
    file->put ('t');
  else
    file->put ("t ");
...
void DratTracer::add_comment (const vector<int> &clause) {
  if (file->closed ())
    return;
  drat_add_comment (clause);
}

void DratTracer::drat_add_orthogonality_clause (const vector<int> &clause) {
  if (binary)
    file->put ('o');
  else
    file->put ("o ");
...
void DratTracer::add_orthogonality_clause (const vector<int> &clause) {
  if (file->closed ())
    return;
  drat_add_orthogonality_clause (clause);
}
