...
def maple_to_edges(input, v):
    # Convert string to list by removing brackets and splitting by commas
    str_lst = input.strip('[]').split(',')
    
    # Create all possible edges between vertices (i,j) where i < j
    edge_lst = []
    for j in range(0, v):
        for i in range(0, j):
            edge_lst.append((i,j))
    
    # Create the actual edges list from the positive indicators
    actual_edges = []
    for i in str_lst:
        index = int(i.strip()) - 1  # -1 because edge indices are 1-based
        actual_edges.append(edge_lst[index])
    return actual_edges
...
def main_single_graph(g, order, index, using_subgraph, normalize=False, output_unsat_f="output_unsat_f", output_sat_f="output_sat_f", verify=False):
    ...
    if using_subgraph:
        ...
        file_path = os.path.join(current_dir, "min_nonembed_graph_10-12.txt")
        ...
        solved = False
        while not solved:
            solved = determine_embed(graph_dict, assignments, g, order, index, using_subgraph, normalize, output_unsat_f, output_sat_f, verify) #write the file
            index += 1
    else:
        graph_dict = {}
        for v in list(G.nodes()):
