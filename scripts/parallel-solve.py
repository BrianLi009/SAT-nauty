...
def run_command(command):
    ...
    try:
        ...
        if "UNSAT" in stdout.decode():
            ...
            command = f"cube('{file_to_cube}', 'N', 0, {mg}, '{orderg}', {numMCTSg}, queue, '{cutoffg}', {cutoffvg}, {dg}, 'True', {partition_val_g}, {ortho_g}, '{cubing_mode_g}', '{solveaftercubeg}')"
...
def worker(queue):
    while True:
        ...
        if args is None:
            queue.task_done()
            break
        if args.startswith("./solve-verify.sh") or args.startswith("./cadical-rcl/build/cadical"):
            run_command(args)
        else:
            run_cube_command(args)
        queue.task_done()

def cube(original_file, cube, index, m, order, numMCTS, queue, cutoff='d', cutoffv=5, d=0, extension="False", partition=None, ortho=False, cubing_mode="ams", solveaftercube="True", complex_mode=False, vectors_file=None):
    print(f"Debug: Cube parameters - file:{original_file}, cube:{cube}, index:{index}, m:{m}, order:{order}, numMCTS:{numMCTS}, cutoff:{cutoff}, cutoffv:{cutoffv}, d:{d}, partition:{partition}, ortho:{ortho}, complex:{complex_mode}, vectors_file:{vectors_file}", flush=True)
    ...
    # Prepare simplify command with new parameter structure
    if cube != "N":
        ...
        simplify_cmd = f"./simplification/simplify-by-conflicts.sh {cube}{index}.cnf --order {order} --conflicts 1000"
        if complex_mode:
            simplify_cmd += " --complex"
        if vectors_file:
            simplify_cmd += f" --vectors-file {vectors_file}"
        file_to_cube = f"{cube}{index}.cnf.simp"
        simplog_file = f"{cube}{index}.cnf.simplog"
        file_to_check = f"{cube}{index}.cnf.ext"
    else:
        simplify_cmd = f"./simplification/simplify-by-conflicts.sh {original_file} --order {order} --conflicts 1000"
        if complex_mode:
            simplify_cmd += " --complex"
        if vectors_file:
            simplify_cmd += f" --vectors-file {vectors_file}"
    ...
    # Add partition and ortho parameters if specified
    if partition:
        simplify_cmd += f" --partition {partition}"
    if ortho:
        simplify_cmd += " --ortho"
    
    print(f"Debug: Executing command: {simplify_cmd}", flush=True)
    subprocess.run(simplify_cmd, shell=True)
    ...
    if cutoff == 'd':
        ...
        if d >= cutoffv:
            ...
            if solveaftercube == 'True':
                files_to_remove = [f'{cube}{index}.cnf']
                remove_related_files(files_to_remove)
                
                # Updated solve command with new parameter structure
                solve_cmd = f"./solve-verify.sh --order {order}"
                if partition:
                    solve_cmd += f" --partition {partition}"
                if ortho:
                    solve_cmd += " --ortho"
                if complex_mode:
                    solve_cmd += " --complex"
                if vectors_file:
                    solve_cmd += f" --vectors-file {vectors_file}"
                solve_cmd += f" {file_to_cube}"
                
                print(f"Debug: Depth cutoff reached, adding solve task to queue: {solve_cmd}", flush=True)
                queue.put(solve_cmd)
            return
    if cutoff == 'v':
        ...
        if var_removed >= cutoffv:
            ...
            if solveaftercube == 'True':
                ...
                print(f"Debug: Variable cutoff reached, adding solve task to queue: {solve_cmd}", flush=True)
                queue.put(solve_cmd)
            return
    ...
    command1 = f"cube('{original_file}', '{next_cube}', 1, {m}, '{order}', {numMCTS}, queue, '{cutoff}', {cutoffv}, {d}, 'False', {partition}, {ortho}, '{cubing_mode}', '{solveaftercube}', {complex_mode}, '{vectors_file}')"
    command2 = f"cube('{original_file}', '{next_cube}', 2, {m}, '{order}', {numMCTS}, queue, '{cutoff}', {cutoffv}, {d}, 'False', {partition}, {ortho}, '{cubing_mode}', '{solveaftercube}', {complex_mode}, '{vectors_file}')"
...
def main(order, file_name_solve, m, cubing_mode="ams", numMCTS=2, cutoff='d', cutoffv=5, solveaftercube='True', timeout=sys.maxsize, partition=None, ortho=False, complex_mode=False, vectors_file=None):
    print(f"Debug: Main parameters - order:{order}, file:{file_name_solve}, m:{m}, mode:{cubing_mode}, numMCTS:{numMCTS}, cutoff:{cutoff}, cutoffv:{cutoffv}, partition:{partition}, ortho:{ortho}, complex:{complex_mode}, vectors_file:{vectors_file}", flush=True)
    ...
    global queue, orderg, numMCTSg, cutoffg, cutoffvg, dg, mg, solveaftercubeg, file_name_solveg, cubing_mode_g, partition_val_g, ortho_g, complex_mode_g, vectors_file_g, remove_file
    ...
    partition_val_g = partition
    ortho_g = ortho
    complex_mode_g = complex_mode
    vectors_file_g = vectors_file
    remove_file = True
    ...
    with open(file_name_solve, 'r') as file:
        ...
        if first_line.startswith('p cnf'):
            ...
            cube(file_name_solve, "N", 0, m, order, numMCTS, queue, cutoff, cutoffv, d, 'False', partition, ortho, cubing_mode, solveaftercube, complex_mode, vectors_file)
        else:
            ...
            for instance in instance_lst:
                # Create solve command with new parameter structure
                solve_cmd = f"./solve-verify.sh --order {order}"
                if partition:
                    solve_cmd += f" --partition {partition}"
                if ortho:
                ...
                if vectors_file:
                    solve_cmd += f" --vectors-file {vectors_file}"
                solve_cmd += f" {instance}"
                
                print(f"Debug: Adding to queue: {solve_cmd}", flush=True)
                queue.put(solve_cmd)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        epilog='Example usage: python3 parallel-solve.py 17 instances/ks_17.cnf -m 136 --partition 25 --ortho --complex --vectors-file vectors.txt'
    )
    ...
    parser.add_argument('--partition', type=int, 
                        help='Partition value for CaDiCaL')
    parser.add_argument('--ortho', action='store_true',
                        help='Enable ortho mode for CaDiCaL')
    parser.add_argument('--complex', action='store_true',
                        help='Enable complex mode for CaDiCaL')
    parser.add_argument('--vectors-file', type=str,
                        help='Vectors file for CaDiCaL')
    ...
    main(args.order, args.file_name_solve, args.m, args.cubing_mode,
         ...
         args.timeout, args.partition, args.ortho, args.complex, args.vectors_file)